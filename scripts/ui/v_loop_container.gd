...
func _notification(what: int) -> void:
	if what == NOTIFICATION_SORT_CHILDREN:
		...
		var total_height := control_count * (item_height + separation) - separation
		if total_height <= size.y:
			_v_layout_sort(total_height)
		else:
...
func _v_layout_sort(total_height: float) -> void:
	var py := (size.y - total_height) / 2
	for child in get_children():
		var c := child as Control
		if c == null: continue
		var w := size.x
